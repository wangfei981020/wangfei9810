...
def get_view_jobs(server, view_name):
    """返回指定视图中的所有 Jenkins 作业"""
    try:
        view = server.get_jobs(view_name)
        return view
    except jenkins.JenkinsException as e:
        print(f"Error: Failed to fetch jobs for view '{view_name}'. {e}")
        raise
...
def update_job_config(config_data):
    ...
    for j in view_jobs:
        ...
        # if 'G32-' in j['name'] and j['name'] != 'G32-批量更新勾选job到最新版本' and j['name'] == 'G32-base-client-backend':
