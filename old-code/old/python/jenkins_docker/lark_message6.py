...
def get_lark_id_from_nacos():
    ...
    NACOS_PORT = 8847  # Nginx 配置的端口，实际是代理到 Nacos 服务的端口
    # Nacos 登录账号和密码
    ...
    for attempt in range(retry_count):
        try:
            logger.info(f"第 {attempt + 1} 次尝试连接 Nacos 服务...")
            # 获取指定 data_id 和 group 的配置
            config = client.get_config(data_id, group)
            yaml_config = yaml.safe_load(config)
            logger.info("成功获取到 Nacos 配置!!!")
            return yaml_config
        except (RequestException, Timeout, ConnectionError) as e:
            # 捕获连接、超时等网络相关错误
            logger.error(f"获取配置失败，网络错误或连接问题 (第 {attempt + 1} 次尝试): {e}")
        except NacosRequestException as e:
            # 捕获 Nacos 服务不可用的错误
            logger.error(f"获取配置失败，Nacos 服务不可用 (第 {attempt + 1} 次尝试): {e}")
