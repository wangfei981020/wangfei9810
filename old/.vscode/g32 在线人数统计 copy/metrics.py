        ...
        for i in nacos_info:
            if i['project'] == "ppu":
                ...
                if i.get('game') is not None:
                    game_info = i.get('game')
                    for game in game_info:
                        gameId_g32_map[game['gameId']['name']] = game['gameId']['value']
                        for room in game['gameId']['gameRoomIds']:
                            gameRoomId_g32_map[room['roomId']] = room['value']
                ...
                if "data" in data and data["data"] is not None:
                    for item in data["data"]:
                        ...
                        gameRoomId = item.get("gameRoomId", "")
                        gameId = item.get("gameId", "")
                        if gameRoomId in gameRoomId_g32_map:
                            item["gameRoomId"] = gameRoomId_g32_map[gameRoomId]
                        if gameId in gameId_g32_map:
                            item["gameId"] = gameId_g32_map[gameId]
         ...
         #       logger.info(f"Parsed values for project other {project}: {values}")
        # # 处理特定项目的额外逻辑
        if project in ["ppu"]:
            print('ppu111',project)
            ...
            logger.info(f"Parsed values for project ppu {project}: {values}")
            return values
        # # 处理特定项目的额外逻辑
        elif project in ["g32"]:
            print('g32_values',g32_values)
            print('gameRoomId_g32_map',gameRoomId_g32_map)
            keys_to_delete = [key for key, value in gameRoomId_g32_map.items() if any(value == i['vid'] for i in values)]
            print('keys_to_delete',keys_to_delete)
            for key in keys_to_delete:
                del gameRoomId_g32_map[key]
            ...
            logger.info(f"Parsed values for project g32  {project}: {g32_values}")
            return g32_values
        #print('1111')
