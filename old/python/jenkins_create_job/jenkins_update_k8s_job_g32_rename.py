...
def get_view_jobs(server, view_name):
    ...
    try:
        ...
        print(f"可用视图列表: {[v['name'] for v in all_views]}")  # 输出所有视图
        raise
...
def update_job_config(config_data):
    ...
    for job in jobs:
        ...
        # print(f"\n处理作业: {original_name}")
        # if original_name == "G32-baccarat-server-backend-test-test":
        if original_name != "G32-baccarat-resource":
            ...
            try:
                ...
                # 重命名 Job
                new_job_name = f"k8s-{config_data['projects']['project_name']}_{trimmed_name}"
                server.rename_job(original_name, new_job_name)
                print(f"\033[92m成功重命名: {original_name} -> {new_job_name}\033[0m")

                # 删除旧的 Job
                server.delete_job(original_name)
                print(f"\033[91m删除旧的 Job: {original_name}\033[0m")

            except jenkins.JenkinsException as e:
                print(f"\033[91m配置更新失败: {original_name} - {e}\033[0m")
            except ET.ParseError as e:
                print(f"\033[91mXML 格式验证失败: {original_name} - {e}\033[0m")

def main():
    # 读取配置文件
    try:
        config_data = read_config('config.yaml')  # 配置文件路径
    except Exception as e:
        print(f"程序终止: {e}")
        return

    # 调用更新作业配置的函数
    update_job_config(config_data)
