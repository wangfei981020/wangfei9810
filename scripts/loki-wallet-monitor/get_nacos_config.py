...
NACOS_CONFIG = {
    ...
    'password': '123456',
    'namespace': 'devops',
    'data_id': os.environ.get('NACOS_DATA_ID', 'g32-prod-loki-wallet.yaml'),
    'group': 'DEFAULT_GROUP',
    # 构建缓存文件路径为相对路径 'nacos-data/snapshot/players-exporter.yml+DEFAULT_GROUP+devops'
    'cache_file': os.path.join('nacos-data', 'snapshot', 'g32-prod-loki-wallet.yaml+DEFAULT_GROUP+devops')  # 使用 os.path.join 来保证路径的跨平台兼容性
}
...
def get_config_from_nacos(client: nacos.NacosClient) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"nacos配置: {yaml_config}")
