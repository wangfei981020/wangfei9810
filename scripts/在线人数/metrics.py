...
class GameDataProcessor:
    ...
    def _process_game_onlie(self,response,roomId,base_entries):
        onle_list = set()
        # online_list = set()
        onle_dict = {}
        num = 0
        if response.get("data"):
            for j in  response.get('data'):
                if roomId.get('roomId') == j.get('gameRoomId'):
                    print('asaasssssss222-----------------------',roomId)
                    for k in j.get('clientSessionList'):
                        if k.get('userId') not in onle_list:
                            num += 1
                            # onle_list.append(k.get('userId'))
                            onle_list.add(k.get('userId'))
                            roomname = roomId.get('value')
                            onle_dict[roomname] = len(onle_list)
                            print('onle_dict',onle_list)

        if onle_dict:
            print('1111',onle_dict)
            return onle_dict
        else:
            return {}
    ...
    def _process_game_data(self, project_config: dict) -> List[dict]:
        ...
        for game_config in project_config.get('game', []):
            game_id = game_config['gameId']['value']
            # print('1111',game_config)
            for room_config in game_config['gameId'].get('gameRoomIds', []):
                print('2222',room_config)
                print('----------------------------------------------------------------------------------------')
                data_onlie = self._process_game_onlie(response,room_config,base_entries)
                if data_onlie:
                    # print('3333-------------------',room_config)
                    entry = {
                        'project': project_name,
                        'vid': room_config['value'],
                        'gmtype': game_id,
                        room_config['roomId']: room_config['value'],
                        'players': data_onlie.get(room_config['value'])  # 默认人数为 0
                    }
                    base_entries.append(entry)
                else:
                    entry = {
    ...
    def _process_game_result(self, project_config: dict) -> List[dict]:
        ...
        if not response or not response.get('result') :
            if response is None or not response.get('body'):
                logger.error(f"{project_name} response 为 None")
                return []
            else:
                data_dict = response.get('body')
                print('1111',data_dict)
                if data_dict:
