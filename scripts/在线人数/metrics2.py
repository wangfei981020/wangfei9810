...
class GameDataProcessor:
    ...
    def _send_request(self, method: str, url: str, headers: dict = None) -> Optional[dict]:
        try:
            merged_headers = {**self.session.headers, **(headers or {})}
            response = self.session.request(method, url, headers=merged_headers,timeout=(1, 1))
    ...
    def _process_game_data(self, project_config: dict) -> List[dict]:
        ...
        d = project_config.get('headers')
        print('dddd',d)
        print('dddd',project_config.get('project'))
    ...
    def _process_game_result(self, project_config: dict) -> List[dict]:
        ...
        d = project_config.get('headers')
        print('sss',d)
        print('sss',project_config.get('project'))
